...
@Service("wordAnalyser")
public class JWNLWordAnalyser implements WordAnalyser {

    private Dictionary dictionary = null;

    public JWNLWordAnalyser() throws JWNLException {
        this.dictionary = Dictionary.getDefaultResourceInstance();
    }

    public String morphologicalAnalysis(String phrase) {
        ...
        try {
            IndexWord iw = dictionary.lookupIndexWord(POS.VERB, phrase);
            if (iw != null)
                return iw.getLemma();
    ...
    private String listOp(IndexWord word) throws JWNLException {
        ...
        return word.getLemma() + ":" + hypernyms.toString();
    }

    private String treeOp(IndexWord word) throws JWNLException {
        // Get all the hyponyms (children) of the first sense of <var>word</var>
        PointerTargetTree hyponyms = PointerUtils.getHyponymTree(word.getSenses().get(0));
        return word.getLemma() + ":" + hyponyms.toString();
    }

    private void asymmetricRelationshipOp(IndexWord start, IndexWord end) throws JWNLException, CloneNotSupportedException {
    ...
    private void symmetricRelationshipOp(IndexWord start, IndexWord end) throws JWNLException, CloneNotSupportedException {
